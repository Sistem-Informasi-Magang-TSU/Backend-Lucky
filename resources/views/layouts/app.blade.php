...
<body class="bg-white font-sans text-gray-800 flex min-h-screen">
    ...
    <aside class="w-64 bg-tsu-teal flex-shrink-0 flex flex-col fixed h-full z-30 overflow-y-auto shadow-xl">
        ...
        <nav class="flex-1 px-4 space-y-3 mt-2">
            ...
            <a href="{{ route('program.index') }}" class="flex items-center gap-3 px-6 py-3 rounded-full font-bold {{ $isProgram ? $activeClass : $inactiveClass }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div id="logoutModal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black bg-opacity-40 backdrop-blur-sm transition-opacity duration-300">
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 transform transition-transform duration-300 scale-95" id="logoutModalContent">
            ...
            <div class="flex gap-4">
                <button type="button" id="cancelLogout" class="flex-1 px-4 py-3 bg-gray-100 text-gray-600 font-bold rounded-2xl hover:bg-gray-200 transition">Kembali</button>
                <form method="POST" action="{{ route('logout') }}" class="flex-1">
    @csrf
    <button type="submit"
        class="w-full px-4 py-3 bg-tsu-red text-white font-bold rounded-2xl hover:bg-red-700 transition">
        Keluar
    </button>
</form>
